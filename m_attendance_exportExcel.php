...
if(isset($_GET['term'])) {
	...
	if($_GET['term']=='') {
		$sqlstr="select stuno, stuname, grade,chTerm,count(case when yqChair is not null then 1 else 0 end ) as total from yq_info a left join yq_clickcard b on a.stuno=b.yqNUm left join yq_chair c on b.yqChair=c.id where  grade='$strGrade' GROUP BY stuno  $strBuilder ";
		
	}else {
		$sqlstr="select stuno, stuname, grade,chTerm,count(case when yqChair is not null then 1 else 0 end ) as total from yq_info a left join yq_clickcard b on a.stuno=b.yqNUm left join yq_chair c on b.yqChair=c.id where  grade='$strGrade' and chTerm = '$strTerm' GROUP BY stuno  $strBuilder ";
		
// 		$sqlstr="select stuno, stuname, grade,chTerm,count(case when yqChair is not null then 1 else 0 end ) as total from yq_clickcard,yq_info,yq_chair where yq_clickcard.yqNum=yq_info.stuno and yq_clickcard.yqChair=yq_chair.id and grade='$strGrade' and chTerm = '$strTerm' GROUP BY stuno  $strBuilder ";
